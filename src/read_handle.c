...
int mqtt3_hilight_packet_handle(struct mosquitto_db *db, struct mosquitto *context)
{
	if (!context) return MOSQ_ERR_INVAL;
	...
	case PUBCOMP:
		return _mosquitto_handle_pubackcomp(db, context, "PUBCOMP");
	case PUBLISH:
		return mqtt3_hilight_handle_publish(db, context);
	case PUBREC:
		return _mosquitto_handle_pubrec(context);
	case PUBREL:
		return _mosquitto_handle_pubrel(db, context);
...
int mqtt3_handle_publish(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	printf("No Hilight Recevied Publish db 1 \n");
...
int mqtt3_hilight_handle_publish(struct mosquitto_db *db, struct mosquitto *context)
{
	char *topic;
	void *payload = NULL;
	uint32_t payloadlen;
	...
	printf("Hilight Recevied Publish db 2 \n");
	printf("Hilight enqueue !!\n");
	//enqueue 수정  @@
	element data;
	...
	db = _mosquitto_get_db(); //db change
	...
	switch (qos) {
	case 0:
		if (mqtt3_hilight_db_messages_queue(db, context->id, topic, qos, retain, &stored)) rc = 1;
		break;
	case 1:
		if (mqtt3_hilight_db_messages_queue(db, context->id, topic, qos, retain, &stored)) rc = 1;
		if (_mosquitto_send_puback(context, mid)) rc = 1;
