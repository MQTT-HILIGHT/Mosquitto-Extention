...
int mosquitto_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max)
{
	...
	int hilight_count = 0; //normal queue 갯 수 셈 hilight
	...
	while(run) { //진짜 메인 loop 스타트!
		...
		//Sleep(2000); //수정
		//printf("시작  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		hilight_send_data(); //send urgency data
		...
		if(db->config->persistent_client_expiration > 0 && now_time > expiration_check_time){
			HASH_ITER(hh_id, db->contexts_by_id, context, ctxt_tmp){
				if(context->sock == INVALID_SOCKET && context->clean_session == 0){
					...
					//타임아웃 내에 즉 expiration_check_time내에 검증 완료된 사용자는 id를 true로
...
#ifdef WITH_SYS_TREE
						g_clients_expired++;	//만료된 사용자를 증가시키고
#endif
						context->clean_session = true;
						context->state = mosq_cs_expiring;
						do_disconnect(db, context);
						//만료된 사용자들은 clean 시켜 버림
...
#else
		fdcount = WSAPoll(pollfds, pollfd_index, 100);  //https://www.slideshare.net/jacking/winsock-api-wsapoll-fast-loopback   //start control
#endif
		if(fdcount == -1){
			_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error in poll: %s.", strerror(errno));
		}else{
			loop_handle_reads_writes(db, pollfds); //해당하는 pollfd 설정

			//각각의 pollfds의 반환 결과를 검증함
			for(i=0; i<listensock_count; i++){
...
//해당하는 pollfds 설정을 다 해줌  hashiter를 통해 pollfds로 해줌!
static void loop_handle_reads_writes(struct mosquitto_db *db, struct pollfd *pollfds)
{
	struct mosquitto *context, *ctxt_tmp;
	int err;
	...
	HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){
		...
		if(pollfds[context->pollfd_index].revents & POLLOUT){
			...
			if(_mosquitto_packet_write(context)){  //write control
				do_disconnect(db, context);
				continue;
			}
		}
	...
	HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){ //hashiter control
		if(context->pollfd_index < 0){
			continue;
		}
#ifdef WITH_WEBSOCKETS
...
#endif
			do{
				if(_mosquitto_packet_read(db, context)){  //read control
					do_disconnect(db, context);
					continue;
				}
