...
static void hilight_loop_handle_reads_writes(struct mosquitto_db *db, struct pollfd *pollfds); //수정
...
#ifdef WIN32
	pollfd_max = 2000; // max control
#else
	pollfd_max = getdtablesize();  // max control
#endif
...
//hilight code
int hilight_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max)
{
#ifdef WITH_SYS_TREE
	time_t start_time = mosquitto_time();
#endif
...
#ifdef WIN32
	pollfd_max = 2000; // max control _getmaxstdio()
#else
	pollfd_max = getdtablesize(); // max control
#endif
	...
	while (run) {//진짜 메인 loop 스타트!
		...
		if (fdcount == -1) {
			...
			hilight_loop_handle_reads_writes(db, pollfds); //수정
...
static void hilight_loop_handle_reads_writes(struct mosquitto_db *db, struct pollfd *pollfds)
{
	struct mosquitto *context, *ctxt_tmp;
	int err;
	socklen_t len;
...
#endif
			do {
				if (_hilight_packet_read(db, context)) { //수정
					do_disconnect(db, context);
					continue;
				}
			} while (SSL_DATA_PENDING(context));
